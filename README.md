...
一个单页聊天前端。iOS 液态玻璃质感、多会话、六套主题、可发图片、可添加到主屏幕。
...
薰衣草紫 · 青柠 · 奶茶 · 胭脂 · 落花 · 深夜。在侧栏底部「外观」切换。

新增主题只需在 `index.html` 顶部的 CSS 里加一组 `[data-theme="名字"]` 变量，再往侧栏加一个 `.sw` 按钮。

`--blob-op` 和 `--blob-blur` 控制背景那三团色的浓度和虚化程度，也按主题给。胭脂用的是 `.46 / 96px`——比其他主题淡、比其他主题散，饱和度一高就显得塑料。

「落花」的底子是白的：色团压到 `.34 / 110px`，只在四角留一点晕。白底上白玻璃会消失，所以这套的 `--glass-line` 是粉的，气泡和输入条靠那道发丝线站住，不靠明度差。

四周飘的花来自 `#petals`，只有 `--petal-op` 非 0 的主题看得见，其余主题它就是一个透明的空层，不用管。位置写死在 JS 里的 `PETALS` 表：随机每次刷新都换一个样子，读起来像页面没稳住。全部贴着两侧和四角，中间那条竖直的阅读区留空；飘动幅度只有十几像素——余光里动一下就够了，动大了会跟正在读的字抢注意力。`prefers-reduced-motion` 下不动。

## 输入条

输入条是绝对定位浮在消息之上的，不占布局高度；`#scroll` 用 `padding-bottom` 给它让出位置。所以消息会从它底下滑过去，`backdrop-filter` 才有东西可模糊——贴在底边时它背后只有背景色，玻璃等于白做。

`#composer` 本身是 `pointer-events:none`，只有子元素恢复成 `auto`，这样输入条两侧的空白不挡滚动。
